...
def term_input(list_of_cards: list, number_value: int):
    term_value = input()
    for card_object in list_of_cards:
        if term_value == card_object.term:
            print(f'The term "{term_value}" already exists. Try again')
            return term_input(list_of_cards, number_value)
    return term_value


def definition_input(list_of_cards: list, number_value: int):
    definition_value = input()
    for card_object in list_of_cards:
        if definition_value == card_object.definition:
            print(f'The definition "{definition_value}" already exists. Try again:')
            return definition_input(list_of_cards, number_value)
